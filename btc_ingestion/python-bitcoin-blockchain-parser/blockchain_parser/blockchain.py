...
def get_block_filename(file_number):
    return "blk%05d.dat" % file_number

def get_block_undo_filename(file_number):
    return "rev%05d.dat" % file_number
...
class Blockchain(object):
    ...
    def __init__(self, path, cache=None):
        self.path = path
        self.indexPath = path + "/index"
        self.blockIndexes = None

        if cache and os.path.exists(cache):
            # load the block index cache from a previous index
            with open(cache, 'rb') as f:
                self.blockIndexes = pickle.load(f)

        if self.blockIndexes is None:
            ...
            self.blockIndexes = self._filter_orphans(self.blockIndexes)
    ...
    def get_block_index_by_height(self, height):
        return self.blockIndexes[height]

    def get_block_undo(self, height):
        block_index = self.index.get_block_index_by_height(height)
        return BlockUndo(get_block(get_block_undo_filename(block_index.file), block_index.data_Pos))

    def get_block(self, height):
        block_index = self.index.get_block_index_by_height(height)
        return Block(get_block(get_block_filename(block_index.file), block_index.data_Pos))
    ...
    def _filter_orphans(self, blockIndexes):
    ...
    def get_ordered_blocks(self, start=0, end=None):
        ...
        for blkIdx in blockIndexes[start:end]:
            ...
            block = Block(get_block(blkFile, blkIdx.data_pos), blkIdx.height)
       
            blockUndo = None
            try:
                blkUndoFile = os.path.join(self.path, "rev%05d.dat" % blkIdx.file)
                blockUndo = BlockUndo(get_block(blkUndoFile, blkIdx.undo_pos))
            except AttributeError:
                pass

            yield block, blockUndo
