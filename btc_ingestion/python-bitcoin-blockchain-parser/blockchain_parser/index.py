...
class DBTransactionIndex(object):
    def __init__(self, raw_hex):
        pos = 0
        self.file_no, i = _read_varint(raw_hex[pos:])
        pos += i
        self.block_pos, i = _read_varint(raw_hex[pos:])
        pos += i
        self.tx_pos, i = _read_varint(raw_hex[pos:])

    def __repr__(self):
        return "DBTransactionIndex(file_no=%d, block_pos=%d, tx_pos=%d)" \
               % (self.file_no, self.block_pos, self.tx_pos)
